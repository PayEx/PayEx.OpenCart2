...
class ModelModuleMobilepay extends Model
{
    /**
    ...
    {
        return $this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "mobilepay_transactions` (
    ...
    {
        $query = sprintf('INSERT INTO `' . DB_PREFIX . 'mobilepay_transactions` (order_id, transaction_id, transaction_status, transaction_data, date) VALUES (%d, %d, %d, "%s", "%s");',
    ...
    {
        $query = sprintf('UPDATE `' . DB_PREFIX . 'mobilepay_transactions` SET is_captured = 1 WHERE transaction_id = %d;',
            $this->db->escape((int)$transaction_id)
        );
        try {
            return $this->db->query($query);
    ...
    {
        $query = sprintf('UPDATE `' . DB_PREFIX . 'mobilepay_transactions` SET is_canceled = 1 WHERE transaction_id = %d;',
            $this->db->escape((int)$transaction_id)
        );
        try {
            return $this->db->query($query);
    ...
    {
        $query = sprintf('UPDATE `' . DB_PREFIX . 'mobilepay_transactions` SET is_refunded = 1, total_refunded = %d WHERE transaction_id = %s;',
