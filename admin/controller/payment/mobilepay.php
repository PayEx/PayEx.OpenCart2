...
class ControllerPaymentMobilepay extends Controller
{
    ...
    protected $_options = array(
        'mobilepay_account_number',
        'mobilepay_encryption_key',
        'mobilepay_mode',
        'mobilepay_transactiontype',
	    'mobilepay_client_language',
        'mobilepay_total',
        'mobilepay_completed_status_id',
        'mobilepay_pending_status_id',
        'mobilepay_canceled_status_id',
        'mobilepay_failed_status_id',
        'mobilepay_refunded_status_id',
        'mobilepay_geo_zone_id',
        'mobilepay_status',
        'mobilepay_sort_order',
    );
    ...
    function index()
    {
        ...
        $this->model_module_mobilepay->createModuleTables();
            ...
            if (isset($this->request->post['action'])) {
                ...
                switch ($this->request->post['action']) {
                    case 'capture':
                        ...
                        // Set Order Status
                        $order_status_id = $data['mobilepay_completed_status_id'];
                    ...
                    case 'cancel':
                        ...
                        // Set Order Status
                        $order_status_id = $data['mobilepay_canceled_status_id'];
                        $this->db->query("UPDATE `" . DB_PREFIX . "order` SET order_status_id = '" . (int)$order_status_id . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
                        $this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$order_id . "', order_status_id = '" . (int)$order_status_id . "', date_added = NOW()");
                    ...
                    case 'refund':
                        ...
                        // Set Order Status
                        $order_status_id = $data['mobilepay_refunded_status_id'];
                        $this->db->query("UPDATE `" . DB_PREFIX . "order` SET order_status_id = '" . (int)$order_status_id . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
                        $this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$order_id . "', order_status_id = '" . (int)$order_status_id . "', date_added = NOW()");
        ...
        $query = sprintf("SELECT * FROM %smobilepay_transactions ORDER BY order_id DESC;", DB_PREFIX);
        ...
        $query = sprintf("SELECT * FROM %smobilepay_transactions WHERE transaction_status = %d AND is_captured <> 1 AND is_canceled <> 1 ORDER BY date DESC;", DB_PREFIX, 3);
		...
		$this->response->setOutput($this->load->view('payment/mobilepay.tpl', $data));
    }
    ...
    protected function validate()
    {
        ...
        $account_number = $this->request->post['mobilepay_account_number'];
        $encryption_key = $this->request->post['mobilepay_encryption_key'];

        if ($account_number && !filter_var($account_number, FILTER_VALIDATE_INT)) {
            $this->error['mobilepay_account_number'] = $this->language->get('error_account_number');
        }

        if (empty($encryption_key)) {
            $this->error['mobilepay_encryption_key'] = $this->language->get('error_encryption_key');
        }
